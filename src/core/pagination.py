...
class Simple(discord.ui.View):
    ...
    async def start(self, ctx: discord.Interaction | commands.Context, pages: list[discord.Embed]):
        ...
        page_counter = SimplePaginatorPageCounter(style=self.PageCounterStyle, TotalPages=len(pages), InitialPage=self.InitialPage)
        self.add_item(self.PreviousButton)
        self.add_item(page_counter)
        self.add_item(self.NextButton)

        self.message = await ctx.reply(embed=pages[self.InitialPage], view=self, ephemeral=self.ephemeral)
