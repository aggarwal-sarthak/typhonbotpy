...
class Leave(commands.Cog):
    ...
    @commands.command(
        description="Leaves The Server With Given ID",
        usage=f"{os.path.basename(__file__)[:-3]} <serverid>",
    )
    @command_enabled()
    async def leave(self, ctx: commands.Context):
        ...
        if guild:
            await guild.leave()
            await ctx.reply(
                f"{tether.constants.success} | Left The Server: `{guild.name}`!"
            )
        else:
            await ctx.reply(
                f"{tether.constants.failed} | No Server Found With Given ID!"
            )
