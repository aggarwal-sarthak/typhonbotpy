...
class Invite(commands.Cog):
    ...
    @commands.command(
        description="Generates Invite Link of a Guild",
        aliases=["invites"],
        usage=f"{os.path.basename(__file__)[:-3]} <guild_id>",
    )
    @command_enabled()
    async def invite(self, ctx: commands.Context, guild: discord.Guild):
        ...
        try:
            ...
            await ctx.reply(
                f"{tether.constants.failed} | Missing Permission To Generate Invite!"
            )
            return
        except discord.HTTPException:
            await ctx.reply(
                f"{tether.constants.failed} | An Error Occurred While Fetching The Information!"
            )
            return
