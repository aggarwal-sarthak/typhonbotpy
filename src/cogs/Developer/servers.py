...
class Servers(commands.Cog):
    ...
    async def servers(self, ctx: commands.Context):
        ...
        for i in range(0, len(data), 20):
            description = "\n".join(f'{j + 1}. {data[j].name} - {data[j].id} - {data[j].member_count}' for j in range(i, min(i + 20, len(data))))
            embed = discord.Embed(title=f'Bot Servers [{len(self.client.guilds)}]', description=f'```{description}```', color=discord.Colour.from_str(tether.color))
