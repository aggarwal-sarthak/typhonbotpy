...
class Snipe(commands.Cog):
    ...
    async def snipe(self, ctx: commands.Context):
        if ctx.channel.id not in self.deleted_messages or not self.deleted_messages[ctx.channel.id]:
            return await ctx.reply(f"{tether.constants.failed} | No Deleted Messages Found In This Channel!")

        embed = discord.Embed(title='Message Found', color=discord.Colour.from_str(tether.color))
