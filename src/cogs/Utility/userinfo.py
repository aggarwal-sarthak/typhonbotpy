...
class Userinfo(commands.Cog):
    ...
    @commands.command(
        description="Returns Information For The Mentioned User",
        aliases=["user", "ui", "about"],
        usage=f"{os.path.basename(__file__)[:-3]} [User]",
    )
    ...
    async def userinfo(self, ctx: commands.Context, member: discord.Member = None):
        ...
        badge_text = " ".join(
            getattr(tether.constants, badge.name, None) for badge in badges
        )
        now = datetime.datetime.now()

        embed.add_field(
            name="**__General Information__**",
            value=f"**Name :** {member}\n**ID :** {member.id}\n**Nickname :** {member.nick}\n**Badges :** {badge_text}\n**Account Creation :** {timeago.format(member.created_at.astimezone(pytz.timezone('Asia/Kolkata')).replace(tzinfo=None),now)}\n**Server Joined :** {timeago.format(member.joined_at.astimezone(pytz.timezone('Asia/Kolkata')).replace(tzinfo=None),now)}",
            inline=False,
        )
        embed.add_field(
            name=f"**__Roles [{len(member.roles)-1}]__**",
            value=(
                ", ".join(roles[:-1])
                if len(str(roles)) < 1024
                else "Too Many Roles To Display!"
            ),
            inline=False,
        )
        embed.add_field(
            name=f"**__Permissions__**",
            value=f'```{", ".join(permissions) if len(str(permissions)) < 1024 else "Too Many Permissions To Display!"}```',
            inline=False,
        )
