...
class Serverinfo(commands.Cog):
    ...
    async def serverinfo(self, ctx: commands.Context):
        ...
        embed.add_field(name="**__Channels__**", value=f"**Total:** {len(ctx.guild.channels)}\n**Channels:** {tether.constants.text} {len(ctx.guild.text_channels)} | {tether.constants.voice} {len(ctx.guild.voice_channels)}\n**Rules Channel:** {ctx.guild.rules_channel.mention if ctx.guild.rules_channel else tether.constants.failed}", inline=False)
        embed.add_field(name="**__Emojis__**", value=f"**Regular:** {sum(1 for emoji in ctx.guild.emojis if not emoji.animated)}\n**Animated:** {sum(1 for emoji in ctx.guild.emojis if emoji.animated)}", inline=False)
        embed.add_field(name="**__Boosts__**", value=f"**Level:** {ctx.guild.premium_tier} [{tether.constants.premium} {ctx.guild.premium_subscription_count} Boosts]\n**Server Booster:** {ctx.guild.premium_subscriber_role.mention if ctx.guild.premium_subscriber_role else tether.constants.failed}", inline=False)
        embed.add_field(name=f"**__Roles [{len(roles)-1}]__**", value=", ".join(roles[:-1]) if len(roles) < 40 else "Too Many Roles To Show!", inline=False)
        if ctx.guild.icon: embed.set_thumbnail(url=ctx.guild.icon.url)
        if ctx.guild.banner: embed.set_image(url=ctx.guild.banner.url)
