...
class Roleinfo(commands.Cog):
    ...
    @commands.command(description='Returns Information About Role', usage=f"{os.path.basename(__file__)[:-3]} <role>")
    @commands.bot_has_permissions(embed_links=True)
    @commands.cooldown(1, 10, commands.BucketType.user)
    @command_enabled()
    async def roleinfo(self, ctx: commands.Context, role: discord.Role=None):
        permissions = ", ".join(sorted([str(perm[0]).replace("_", " ").title() for perm in role.permissions if perm[1]]))
        ...
        embed.add_field(name="**__General Information__**", value=f"**Name :** {role.name}\n**ID :** {role.id}\n**Role Position :** {len(ctx.guild.roles) - role.position}\n**Color :** {role.color}\n**Created At :** {timeago.format(role.created_at.astimezone(pytz.timezone('Asia/Kolkata')).replace(tzinfo=None), datetime.datetime.now())}\n**Mentionable :** {role.mentionable}\n**Hoisted :** {role.hoist}\n**Managed :** {role.managed}", inline=False)
        embed.add_field(name="**__Permissions__**", value=f"```{"Too Many To Show!" if len(permissions) > 1024 else permissions}```", inline=False)
        embed.add_field(name=f"**__Members [{len(role.members)}]__**", value=f"{"```Too Many To Show!```" if len(members) > 1024 else members}", inline=False)
        if role.icon:
