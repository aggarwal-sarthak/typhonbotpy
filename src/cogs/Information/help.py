...
class Help(commands.Cog):
    ...
    async def help(self, ctx: commands.Context, *args: str):
        ...
        prefix = guild_info['prefix'] if guild_info and "prefix" in guild_info else tether.prefix
        print("\n\n\n\n\n\n\n", discord.Colour.from_str((tether.color)))
        if not args:
            ...
            for folder in os.listdir('./src/cogs'):
                if ctx.author.id not in tether.owner_ids and folder == "Developer":
                    continue
                commands_list = [filename[:-3] for filename in os.listdir(f'./src/cogs/{folder}') if filename.endswith('.py')]
                embed.add_field(name=f"{folder}", value=f"`{'`, `'.join(commands_list)}`", inline=False)
        ...
        desc = (f'**Description:** `{cmd.description or "None"}`\n'
        ...
        embed = discord.Embed(title=f"Command: {cmd.name}", color=discord.Colour.from_str(tether.color), description=desc)
