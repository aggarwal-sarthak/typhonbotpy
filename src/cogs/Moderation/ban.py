    ...
    async def ban(self, ctx: commands.Context, user: discord.Member, *reason: str):
        ...
        if ctx.guild.get_member(self.client.user.id).top_role.position <= role.position:
            return await ctx.reply(
                f"{tether.constants.failed} | My Role Isn't High Enough To Ban!"
            )
            ...
            return await ctx.reply(
                f"{tether.constants.failed} | Your Role Isn't High Enough To Ban!"
            )
        ...
        if view.value:
            ...
            return await ctx.reply(
                f"{tether.constants.success} | <@{user.id}> Was Banned successfully!"
            )
        ...
        if view.value is False:
            if msg:
                await msg.delete()
            return await ctx.reply(
                f"{tether.constants.success} | Ban Cancelled Successfully!"
            )
